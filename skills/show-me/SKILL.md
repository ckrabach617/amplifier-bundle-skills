...
- **Restless-Old-Brian (ROB) — *the critical seam*.** ROB and SM both refuse to take "done" on faith, but they own different questions. ROB asks *"is it real — proven end-to-end, in the right order?"* and answers it *himself*: he reads the logs, runs the chain, checks the order. SM asks the one thing ROB doesn't: *"was this invented, and does the evidence shown actually demonstrate the claim?"* — the authenticity of the evidence itself. A build ROB proves works end-to-end can still carry a fabricated figure or a screenshot that demonstrates nothing; that gap is the claimed seam. (SM's "you must prove it's real *for me*, because I never can" is the characteristic voice — but the intended axis is the evidence, not the reader.) **This seam is contested:** ROB's gate already demands "proven end-to-end as a user would" and forbids a fabricated "works," so a rigorously applied ROB covers much of SM's ground. If the only thing separating them is *who* is reading the result, that is an audience difference, not an axis — and SM should be merged into ROB rather than kept.
